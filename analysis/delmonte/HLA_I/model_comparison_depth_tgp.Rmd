...
delmonte_pairs_I$normalized_tgp <- (log10(delmonte_pairs_I$tgp) + 5) / (-5)

emerson_pairs_I[emerson_pairs_I$normalized_tgp > 1,]$normalized_tgp <- 1
delmonte_pairs_I[delmonte_pairs_I$normalized_tgp > 1,]$normalized_tgp <- 1
```
...
```
```{r}
model <- glm(as.factor(class) ~ score + normalized_tgp + score*normalized_tgp, data = delmonte_pairs_I, family = binomial)
predicted <- predict(model, type = "response")
delmonte_pairs_I$combined_score <- predicted
```

```{r}
roc_delmonte_tgp <- roc(delmonte_pairs_I$class, delmonte_pairs_I$normalized_tgp)
...
roc_delmonte_depth <- roc(delmonte_pairs_I$class, delmonte_pairs_I$score)
...
roc_delmonte_combined_score <- roc(delmonte_pairs_I$class, delmonte_pairs_I$combined_score)
